...
PROMPT_VECTOR_TAGS = """
你是一个高级 RAG 数据打标专家。
请结合全局知识库与本章【最新】的实体状态，从正文中提取 2~5 个最核心的“画面、设定或战斗情节”片段并打标。
...
【打标规则】(极度重要)
注意：这些标签将被直接进行向量化(Vectorized)并用于未来的语义相似度检索。因此，标签绝对不能是孤立的短词(如"战斗","张三")，必须是【高度概括的描述性语义短句】！
为了保证未来在章节生成时能精准召回该片段，请“宁滥勿缺”，为每个精彩片段生成 4~6 个不同维度的语义标签。

请强制从以下【四个维度】为片段生成标签（如果片段包含该维度）：
1. 【实体交互维度】：明确指出“谁”和“谁”在“哪里”做了“什么”。（例如："张三与李四在黑木崖的生死决战"、"王五首次向赵六透露星火联盟的卧底身份"）
2. 【设定/物品/功法维度】：提取招式、功法、物品的运转机制与细节。（例如："神秘玉佩吸收月光能量的运转规则与发光异象"、"九阳神功第一层的经脉运行路线与反噬效果"）
3. 【战斗/高光画面维度】：描述具体的战斗场面或情绪爆发点。（例如："主角首次使用破空剑法秒杀三阶变异丧尸的战斗画面"、"张三得知身世真相后的绝望与崩溃瞬间"）
4. 【势力/背景维度】：揭示了什么宏观世界观或势力动态。（例如："黑龙商会江南分部被彻底摧毁后的现场惨状遗留"、"废土世界中关于辐射风暴爆发周期的最新设定"）

片段字数控制在 200-500 字，必须是正文中真正有价值、有信息量的高光段落。
